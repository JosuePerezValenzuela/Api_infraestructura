...
export class DashboardFacultadTypeormRepository implements DashboardFacultadRepositoryPort {
  ...
  async getDetailDashboard(
    ...
    const heatmapRows: Array<{
      ...
      SELECT
        h.dia,
        to_char(h.hora_inicio, 'HH24:MI') || '-' || to_char(h.hora_fin, 'HH24:MI') AS franja,
        COUNT(*)::int AS slots_ocupados,
        COUNT(*)::int AS slots_totales,
        100::numeric AS pct_ocupacion
      FROM infraestructura.horarios h
      INNER JOIN infraestructura.ambientes a ON a.id = h.ambiente_id
      INNER JOIN infraestructura.bloques b ON b.id = a.bloque_id
      WHERE b.facultad_id = $1
      GROUP BY h.dia, franja
      ORDER BY h.dia ASC, franja ASC
    `,
      [facultadId, slotMinutes, dias],
    );
    ...
    const ocupacionPorBloqueRows: Array<{
      ...
      SELECT
        b.id AS bloque_id,
        b.nombre AS bloque_nombre,
        COUNT(h.id)::int AS slots_ocupados,
        COUNT(h.id)::int AS slots_totales,
        CASE WHEN COUNT(h.id) = 0 THEN 0 ELSE 100 END AS pct_ocupacion
      FROM infraestructura.bloques b
      LEFT JOIN infraestructura.ambientes a ON a.bloque_id = b.id
      LEFT JOIN infraestructura.horarios h ON h.ambiente_id = a.id
      WHERE b.facultad_id = $1
      GROUP BY b.id, b.nombre
    ...
    const ambientesUtilizacionRows: Array<{
      ...
      LEFT JOIN infraestructura.horarios h ON h.ambiente_id = a.id
      WHERE b.facultad_id = $1
      GROUP BY a.id, a.nombre, b.nombre
      ORDER BY pct_ocupacion DESC
    `,
      [facultadId, slotMinutes, dias],
    );
